...
**VFX Compositing Helper** is a production-ready agent skill designed for Animation Production workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install vfxai/VFX-Compositing-Helper

# Or install via pip
pip install skillshub-vfx-compositing-helper

# Or install via npm
npm install @skillshub/vfx-compositing-helper
```
...
skill = load_skill("vfxai/VFX-Compositing-Helper")
...
```json
{
  "skill": "vfxai/VFX-Compositing-Helper",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("vfxai/VFX-Compositing-Helper", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("vfxai/VFX-Compositing-Helper"),
...
```bash
# Clone the skill
skillshub clone vfxai/VFX-Compositing-Helper
cd VFX-Compositing-Helper
...
**Built with ❤️ by [vfxai](https://skillshub.dev/vfxai) on [SkillsHub](https://skillshub.dev)**
