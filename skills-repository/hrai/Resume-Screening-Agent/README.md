...
**Resume Screening Agent** is a production-ready agent skill designed for Human Resources workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install hrai/Resume-Screening-Agent

# Or install via pip
pip install skillshub-resume-screening-agent

# Or install via npm
npm install @skillshub/resume-screening-agent
```
...
skill = load_skill("hrai/Resume-Screening-Agent")
...
```json
{
  "skill": "hrai/Resume-Screening-Agent",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("hrai/Resume-Screening-Agent", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("hrai/Resume-Screening-Agent"),
...
```bash
# Clone the skill
skillshub clone hrai/Resume-Screening-Agent
...
**Built with ❤️ by [hrai](https://skillshub.dev/hrai) on [SkillsHub](https://skillshub.dev)**
