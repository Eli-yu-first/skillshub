...
> Matches patients to eligible clinical trials based on diagnosis, demographics, and inclusion criteria

## Overview

**Clinical Trial Matcher** is a production-ready agent skill designed for AI development workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install trialai/Clinical-Trial-Matcher

# Or install via pip
pip install skillshub-clinical-trial-matcher

# Or install via npm
npm install @skillshub/clinical-trial-matcher
```
...
skill = load_skill("trialai/Clinical-Trial-Matcher")
...
```json
{
  "skill": "trialai/Clinical-Trial-Matcher",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("trialai/Clinical-Trial-Matcher", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("trialai/Clinical-Trial-Matcher"),
...
```bash
# Clone the skill
skillshub clone trialai/Clinical-Trial-Matcher
...
**Built with ❤️ by [trialai](https://skillshub.dev/trialai) on [SkillsHub](https://skillshub.dev)**
