...
**Budget Tracker Agent** is a production-ready agent skill designed for Finance & Accounting workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install budgetbot/Budget-Tracker-Agent

# Or install via pip
pip install skillshub-budget-tracker-agent

# Or install via npm
npm install @skillshub/budget-tracker-agent
```
...
skill = load_skill("budgetbot/Budget-Tracker-Agent")
...
```json
{
  "skill": "budgetbot/Budget-Tracker-Agent",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("budgetbot/Budget-Tracker-Agent", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("budgetbot/Budget-Tracker-Agent"),
...
```bash
# Clone the skill
skillshub clone budgetbot/Budget-Tracker-Agent
cd Budget-Tracker-Agent
...
**Built with ❤️ by [budgetbot](https://skillshub.dev/budgetbot) on [SkillsHub](https://skillshub.dev)**
