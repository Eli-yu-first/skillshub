...
**Figma Plugin Helper** is a production-ready tool skill designed for UI/UX Design workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install figmaai/Figma-Plugin-Helper

# Or install via pip
pip install skillshub-figma-plugin-helper

# Or install via npm
npm install @skillshub/figma-plugin-helper
```
...
skill = load_skill("figmaai/Figma-Plugin-Helper")
...
```json
{
  "skill": "figmaai/Figma-Plugin-Helper",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("figmaai/Figma-Plugin-Helper", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("figmaai/Figma-Plugin-Helper"),
...
```bash
# Clone the skill
skillshub clone figmaai/Figma-Plugin-Helper
cd Figma-Plugin-Helper
...
**Built with ❤️ by [figmaai](https://skillshub.dev/figmaai) on [SkillsHub](https://skillshub.dev)**
