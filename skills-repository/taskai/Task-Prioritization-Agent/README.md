...
> Analyzes task lists and prioritizes using Eisenhower matrix, dependencies, and deadline proximity

## Overview

**Task Prioritization Agent** is a production-ready agent skill designed for Office Productivity workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install taskai/Task-Prioritization-Agent

# Or install via pip
pip install skillshub-task-prioritization-agent

# Or install via npm
npm install @skillshub/task-prioritization-agent
```
...
skill = load_skill("taskai/Task-Prioritization-Agent")
...
```json
{
  "skill": "taskai/Task-Prioritization-Agent",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("taskai/Task-Prioritization-Agent", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("taskai/Task-Prioritization-Agent"),
...
```bash
# Clone the skill
skillshub clone taskai/Task-Prioritization-Agent
...
**Built with ❤️ by [taskai](https://skillshub.dev/taskai) on [SkillsHub](https://skillshub.dev)**
