...
**Parts Catalog Search** is a production-ready tool skill designed for Automotive workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install partsai/Parts-Catalog-Search

# Or install via pip
pip install skillshub-parts-catalog-search

# Or install via npm
npm install @skillshub/parts-catalog-search
```
...
skill = load_skill("partsai/Parts-Catalog-Search")
...
```json
{
  "skill": "partsai/Parts-Catalog-Search",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("partsai/Parts-Catalog-Search", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("partsai/Parts-Catalog-Search"),
...
```bash
# Clone the skill
skillshub clone partsai/Parts-Catalog-Search
cd Parts-Catalog-Search
...
**Built with ❤️ by [partsai](https://skillshub.dev/partsai) on [SkillsHub](https://skillshub.dev)**
