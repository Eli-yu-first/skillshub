...
**VoIP Quality Analyzer** is a production-ready tool skill designed for Telecommunications workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install voip-tools/VoIP-Quality-Analyzer

# Or install via pip
pip install skillshub-voip-quality-analyzer

# Or install via npm
npm install @skillshub/voip-quality-analyzer
```
...
skill = load_skill("voip-tools/VoIP-Quality-Analyzer")
...
```json
{
  "skill": "voip-tools/VoIP-Quality-Analyzer",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("voip-tools/VoIP-Quality-Analyzer", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("voip-tools/VoIP-Quality-Analyzer"),
...
```bash
# Clone the skill
skillshub clone voip-tools/VoIP-Quality-Analyzer
...
**Built with ❤️ by [voip-tools](https://skillshub.dev/voip-tools) on [SkillsHub](https://skillshub.dev)**
