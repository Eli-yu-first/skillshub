...
**Policy Draft Assistant** is a production-ready agent skill designed for Official Document Writing workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install policyai/Policy-Draft-Assistant

# Or install via pip
pip install skillshub-policy-draft-assistant

# Or install via npm
npm install @skillshub/policy-draft-assistant
```
...
skill = load_skill("policyai/Policy-Draft-Assistant")
...
```json
{
  "skill": "policyai/Policy-Draft-Assistant",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("policyai/Policy-Draft-Assistant", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("policyai/Policy-Draft-Assistant"),
...
```bash
# Clone the skill
skillshub clone policyai/Policy-Draft-Assistant
cd Policy-Draft-Assistant
...
**Built with ❤️ by [policyai](https://skillshub.dev/policyai) on [SkillsHub](https://skillshub.dev)**
