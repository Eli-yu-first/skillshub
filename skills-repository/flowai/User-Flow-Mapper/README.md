...
**User Flow Mapper** is a production-ready agent skill designed for UI/UX Design workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install flowai/User-Flow-Mapper

# Or install via pip
pip install skillshub-user-flow-mapper

# Or install via npm
npm install @skillshub/user-flow-mapper
```
...
skill = load_skill("flowai/User-Flow-Mapper")
...
skill = load_skill("flowai/User-Flow-Mapper", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("flowai/User-Flow-Mapper"),
...
```bash
# Clone the skill
skillshub clone flowai/User-Flow-Mapper
cd User-Flow-Mapper
...
**Built with ❤️ by [flowai](https://skillshub.dev/flowai) on [SkillsHub](https://skillshub.dev)**
