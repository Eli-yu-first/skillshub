...
**ML Pipeline Debugger** is a production-ready agent skill designed for Machine Learning workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install debugml/ML-Pipeline-Debugger

# Or install via pip
pip install skillshub-ml-pipeline-debugger

# Or install via npm
npm install @skillshub/ml-pipeline-debugger
```
...
skill = load_skill("debugml/ML-Pipeline-Debugger")
...
```json
{
  "skill": "debugml/ML-Pipeline-Debugger",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("debugml/ML-Pipeline-Debugger", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("debugml/ML-Pipeline-Debugger"),
...
```bash
# Clone the skill
skillshub clone debugml/ML-Pipeline-Debugger
...
**Built with ❤️ by [debugml](https://skillshub.dev/debugml) on [SkillsHub](https://skillshub.dev)**
