...
**Content Calendar Planner** is a production-ready agent skill designed for Marketing & SEO workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install calendarai/Content-Calendar-Planner

# Or install via pip
pip install skillshub-content-calendar-planner

# Or install via npm
npm install @skillshub/content-calendar-planner
```
...
skill = load_skill("calendarai/Content-Calendar-Planner")
...
```json
{
  "skill": "calendarai/Content-Calendar-Planner",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("calendarai/Content-Calendar-Planner", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("calendarai/Content-Calendar-Planner"),
...
```bash
# Clone the skill
skillshub clone calendarai/Content-Calendar-Planner
...
**Built with ❤️ by [calendarai](https://skillshub.dev/calendarai) on [SkillsHub](https://skillshub.dev)**
