...
**ICD Code Lookup Agent** is a production-ready agent skill designed for Healthcare & Medical workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install medcode-ai/ICD-Code-Lookup-Agent

# Or install via pip
pip install skillshub-icd-code-lookup-agent

# Or install via npm
npm install @skillshub/icd-code-lookup-agent
```
...
skill = load_skill("medcode-ai/ICD-Code-Lookup-Agent")
...
```json
{
  "skill": "medcode-ai/ICD-Code-Lookup-Agent",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("medcode-ai/ICD-Code-Lookup-Agent", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("medcode-ai/ICD-Code-Lookup-Agent"),
...
```bash
# Clone the skill
skillshub clone medcode-ai/ICD-Code-Lookup-Agent
cd ICD-Code-Lookup-Agent
...
**Built with ❤️ by [medcode-ai](https://skillshub.dev/medcode-ai) on [SkillsHub](https://skillshub.dev)**
