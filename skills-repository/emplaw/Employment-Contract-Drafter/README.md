...
**Employment Contract Drafter** is a production-ready template skill designed for Legal Domain workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install emplaw/Employment-Contract-Drafter

# Or install via pip
pip install skillshub-employment-contract-drafter

# Or install via npm
npm install @skillshub/employment-contract-drafter
```
...
skill = load_skill("emplaw/Employment-Contract-Drafter")
...
```json
{
  "skill": "emplaw/Employment-Contract-Drafter",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("emplaw/Employment-Contract-Drafter", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("emplaw/Employment-Contract-Drafter"),
...
```bash
# Clone the skill
skillshub clone emplaw/Employment-Contract-Drafter
cd Employment-Contract-Drafter
...
**Built with ❤️ by [emplaw](https://skillshub.dev/emplaw) on [SkillsHub](https://skillshub.dev)**
