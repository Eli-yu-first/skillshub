...
**RAG Pipeline Builder** is a production-ready workflow skill designed for NLP workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install ragai/RAG-Pipeline-Builder

# Or install via pip
pip install skillshub-rag-pipeline-builder

# Or install via npm
npm install @skillshub/rag-pipeline-builder
```
...
skill = load_skill("ragai/RAG-Pipeline-Builder")
...
```json
{
  "skill": "ragai/RAG-Pipeline-Builder",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("ragai/RAG-Pipeline-Builder", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("ragai/RAG-Pipeline-Builder"),
...
```bash
# Clone the skill
skillshub clone ragai/RAG-Pipeline-Builder
...
**Built with ❤️ by [ragai](https://skillshub.dev/ragai) on [SkillsHub](https://skillshub.dev)**
