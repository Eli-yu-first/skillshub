...
**Running Coach Agent** is a production-ready agent skill designed for Fitness & Sports workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install runai/Running-Coach-Agent

# Or install via pip
pip install skillshub-running-coach-agent

# Or install via npm
npm install @skillshub/running-coach-agent
```
...
skill = load_skill("runai/Running-Coach-Agent")
...
```json
{
  "skill": "runai/Running-Coach-Agent",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("runai/Running-Coach-Agent", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("runai/Running-Coach-Agent"),
...
```bash
# Clone the skill
skillshub clone runai/Running-Coach-Agent
...
**Built with ❤️ by [runai](https://skillshub.dev/runai) on [SkillsHub](https://skillshub.dev)**
