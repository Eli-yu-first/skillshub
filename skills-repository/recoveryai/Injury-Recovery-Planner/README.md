...
**Injury Recovery Planner** is a production-ready agent skill designed for Fitness & Sports workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install recoveryai/Injury-Recovery-Planner

# Or install via pip
pip install skillshub-injury-recovery-planner

# Or install via npm
npm install @skillshub/injury-recovery-planner
```
...
skill = load_skill("recoveryai/Injury-Recovery-Planner")
...
```json
{
  "skill": "recoveryai/Injury-Recovery-Planner",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("recoveryai/Injury-Recovery-Planner", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("recoveryai/Injury-Recovery-Planner"),
...
```bash
# Clone the skill
skillshub clone recoveryai/Injury-Recovery-Planner
cd Injury-Recovery-Planner
...
**Built with ❤️ by [recoveryai](https://skillshub.dev/recoveryai) on [SkillsHub](https://skillshub.dev)**
