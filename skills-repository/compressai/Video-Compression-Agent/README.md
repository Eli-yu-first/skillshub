...
**Video Compression Agent** is a production-ready tool skill designed for Video Production workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install compressai/Video-Compression-Agent

# Or install via pip
pip install skillshub-video-compression-agent

# Or install via npm
npm install @skillshub/video-compression-agent
```
...
skill = load_skill("compressai/Video-Compression-Agent")
...
```json
{
  "skill": "compressai/Video-Compression-Agent",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("compressai/Video-Compression-Agent", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("compressai/Video-Compression-Agent"),
...
```bash
# Clone the skill
skillshub clone compressai/Video-Compression-Agent
cd Video-Compression-Agent
...
**Built with ❤️ by [compressai](https://skillshub.dev/compressai) on [SkillsHub](https://skillshub.dev)**
