...
**Text Summarization Agent** is a production-ready agent skill designed for NLP workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install summarai/Text-Summarization-Agent

# Or install via pip
pip install skillshub-text-summarization-agent

# Or install via npm
npm install @skillshub/text-summarization-agent
```
...
skill = load_skill("summarai/Text-Summarization-Agent")
...
```json
{
  "skill": "summarai/Text-Summarization-Agent",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("summarai/Text-Summarization-Agent", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("summarai/Text-Summarization-Agent"),
...
```bash
# Clone the skill
skillshub clone summarai/Text-Summarization-Agent
...
**Built with ❤️ by [summarai](https://skillshub.dev/summarai) on [SkillsHub](https://skillshub.dev)**
