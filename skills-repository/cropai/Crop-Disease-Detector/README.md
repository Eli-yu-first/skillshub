...
> Detects crop diseases from leaf images with treatment recommendations and prevention strategies

## Overview

**Crop Disease Detector** is a production-ready agent skill designed for Agriculture workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install cropai/Crop-Disease-Detector

# Or install via pip
pip install skillshub-crop-disease-detector

# Or install via npm
npm install @skillshub/crop-disease-detector
```
...
skill = load_skill("cropai/Crop-Disease-Detector")
...
```json
{
  "skill": "cropai/Crop-Disease-Detector",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("cropai/Crop-Disease-Detector", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("cropai/Crop-Disease-Detector"),
...
```bash
# Clone the skill
skillshub clone cropai/Crop-Disease-Detector
...
**Built with ❤️ by [cropai](https://skillshub.dev/cropai) on [SkillsHub](https://skillshub.dev)**
