...
> Conducts systematic literature reviews with source evaluation, gap analysis, and citation management

## Overview

**Literature Review Agent** is a production-ready agent skill designed for Academic Writing workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install litbot/Literature-Review-Agent

# Or install via pip
pip install skillshub-literature-review-agent

# Or install via npm
npm install @skillshub/literature-review-agent
```
...
skill = load_skill("litbot/Literature-Review-Agent")
...
```json
{
  "skill": "litbot/Literature-Review-Agent",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("litbot/Literature-Review-Agent", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("litbot/Literature-Review-Agent"),
...
```bash
# Clone the skill
skillshub clone litbot/Literature-Review-Agent
...
**Built with ❤️ by [litbot](https://skillshub.dev/litbot) on [SkillsHub](https://skillshub.dev)**
