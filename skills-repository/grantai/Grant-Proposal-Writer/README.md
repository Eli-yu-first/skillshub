...
**Grant Proposal Writer** is a production-ready prompt skill designed for Academic Writing workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install grantai/Grant-Proposal-Writer

# Or add to your agent configuration
skillshub agent add-skill grantai/Grant-Proposal-Writer
```
...
skill = load_skill("grantai/Grant-Proposal-Writer")
...
```json
{
  "skill": "grantai/Grant-Proposal-Writer",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("grantai/Grant-Proposal-Writer", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("grantai/Grant-Proposal-Writer"),
...
```bash
# Clone the skill
skillshub clone grantai/Grant-Proposal-Writer
cd Grant-Proposal-Writer
...
**Built with ❤️ by [grantai](https://skillshub.dev/grantai) on [SkillsHub](https://skillshub.dev)**
