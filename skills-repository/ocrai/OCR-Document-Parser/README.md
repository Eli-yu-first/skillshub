...
**OCR Document Parser** is a production-ready tool skill designed for Computer Vision workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install ocrai/OCR-Document-Parser

# Or install via pip
pip install skillshub-ocr-document-parser

# Or install via npm
npm install @skillshub/ocr-document-parser
```
...
skill = load_skill("ocrai/OCR-Document-Parser")
...
```json
{
  "skill": "ocrai/OCR-Document-Parser",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("ocrai/OCR-Document-Parser", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("ocrai/OCR-Document-Parser"),
...
```bash
# Clone the skill
skillshub clone ocrai/OCR-Document-Parser
...
**Built with ❤️ by [ocrai](https://skillshub.dev/ocrai) on [SkillsHub](https://skillshub.dev)**
