...
**Firewall Rule Optimizer** is a production-ready tool skill designed for Cybersecurity workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install fwai/Firewall-Rule-Optimizer

# Or install via pip
pip install skillshub-firewall-rule-optimizer

# Or install via npm
npm install @skillshub/firewall-rule-optimizer
```
...
skill = load_skill("fwai/Firewall-Rule-Optimizer")
...
```json
{
  "skill": "fwai/Firewall-Rule-Optimizer",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("fwai/Firewall-Rule-Optimizer", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("fwai/Firewall-Rule-Optimizer"),
...
```bash
# Clone the skill
skillshub clone fwai/Firewall-Rule-Optimizer
cd Firewall-Rule-Optimizer
...
**Built with ❤️ by [fwai](https://skillshub.dev/fwai) on [SkillsHub](https://skillshub.dev)**
