...
**SQL Query Optimizer** is a production-ready tool skill designed for Data Science workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install sqlbot/SQL-Query-Optimizer

# Or install via pip
pip install skillshub-sql-query-optimizer

# Or install via npm
npm install @skillshub/sql-query-optimizer
```
...
skill = load_skill("sqlbot/SQL-Query-Optimizer")
...
```json
{
  "skill": "sqlbot/SQL-Query-Optimizer",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("sqlbot/SQL-Query-Optimizer", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("sqlbot/SQL-Query-Optimizer"),
...
```bash
# Clone the skill
skillshub clone sqlbot/SQL-Query-Optimizer
...
**Built with ❤️ by [sqlbot](https://skillshub.dev/sqlbot) on [SkillsHub](https://skillshub.dev)**
