...
**LoRaWAN Network Planner** is a production-ready agent skill designed for IoT & Embedded workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install loraai/LoRaWAN-Network-Planner

# Or install via pip
pip install skillshub-lorawan-network-planner

# Or install via npm
npm install @skillshub/lorawan-network-planner
```
...
skill = load_skill("loraai/LoRaWAN-Network-Planner")
...
```json
{
  "skill": "loraai/LoRaWAN-Network-Planner",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("loraai/LoRaWAN-Network-Planner", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("loraai/LoRaWAN-Network-Planner"),
...
```bash
# Clone the skill
skillshub clone loraai/LoRaWAN-Network-Planner
cd LoRaWAN-Network-Planner
...
**Built with ❤️ by [loraai](https://skillshub.dev/loraai) on [SkillsHub](https://skillshub.dev)**
