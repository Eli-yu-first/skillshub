...
> Analyzes and optimizes database queries with EXPLAIN plan interpretation and index recommendations

## Overview

**Query Performance Tuner** is a production-ready tool skill designed for Database Management workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install tunerai/Query-Performance-Tuner

# Or install via pip
pip install skillshub-query-performance-tuner

# Or install via npm
npm install @skillshub/query-performance-tuner
```
...
skill = load_skill("tunerai/Query-Performance-Tuner")
...
```json
{
  "skill": "tunerai/Query-Performance-Tuner",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("tunerai/Query-Performance-Tuner", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("tunerai/Query-Performance-Tuner"),
...
```bash
# Clone the skill
skillshub clone tunerai/Query-Performance-Tuner
cd Query-Performance-Tuner
...
**Built with ❤️ by [tunerai](https://skillshub.dev/tunerai) on [SkillsHub](https://skillshub.dev)**
