...
> Estimate shipping costs across carriers with dimensional weight and zone calculations

## Overview

**Shipping Cost Estimator** is a production-ready tool skill designed for Supply Chain & Logistics workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install ship-calc/Shipping-Cost-Estimator

# Or install via pip
pip install skillshub-shipping-cost-estimator

# Or install via npm
npm install @skillshub/shipping-cost-estimator
```
...
skill = load_skill("ship-calc/Shipping-Cost-Estimator")
...
```json
{
  "skill": "ship-calc/Shipping-Cost-Estimator",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("ship-calc/Shipping-Cost-Estimator", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("ship-calc/Shipping-Cost-Estimator"),
...
```bash
# Clone the skill
skillshub clone ship-calc/Shipping-Cost-Estimator
cd Shipping-Cost-Estimator
...
**Built with ❤️ by [ship-calc](https://skillshub.dev/ship-calc) on [SkillsHub](https://skillshub.dev)**
