...
**Notion Workflow Automator** is a production-ready rpa skill designed for Office Productivity workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install notionbot/Notion-Workflow-Automator

# Or install via pip
pip install skillshub-notion-workflow-automator

# Or install via npm
npm install @skillshub/notion-workflow-automator
```
...
skill = load_skill("notionbot/Notion-Workflow-Automator")
...
```json
{
  "skill": "notionbot/Notion-Workflow-Automator",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("notionbot/Notion-Workflow-Automator", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("notionbot/Notion-Workflow-Automator"),
...
```bash
# Clone the skill
skillshub clone notionbot/Notion-Workflow-Automator
cd Notion-Workflow-Automator
...
**Built with ❤️ by [notionbot](https://skillshub.dev/notionbot) on [SkillsHub](https://skillshub.dev)**
