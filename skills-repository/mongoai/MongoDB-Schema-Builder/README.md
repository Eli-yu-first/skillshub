...
> Designs MongoDB document schemas with embedding vs referencing decisions and aggregation pipelines

## Overview

**MongoDB Schema Builder** is a production-ready tool skill designed for Database Management workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install mongoai/MongoDB-Schema-Builder

# Or install via pip
pip install skillshub-mongodb-schema-builder

# Or install via npm
npm install @skillshub/mongodb-schema-builder
```
...
skill = load_skill("mongoai/MongoDB-Schema-Builder")
...
```json
{
  "skill": "mongoai/MongoDB-Schema-Builder",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("mongoai/MongoDB-Schema-Builder", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("mongoai/MongoDB-Schema-Builder"),
...
```bash
# Clone the skill
skillshub clone mongoai/MongoDB-Schema-Builder
cd MongoDB-Schema-Builder
...
**Built with ❤️ by [mongoai](https://skillshub.dev/mongoai) on [SkillsHub](https://skillshub.dev)**
