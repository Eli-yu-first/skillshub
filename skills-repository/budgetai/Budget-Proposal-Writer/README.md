...
**Budget Proposal Writer** is a production-ready prompt skill designed for Official Document Writing workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install budgetai/Budget-Proposal-Writer

# Or add to your agent configuration
skillshub agent add-skill budgetai/Budget-Proposal-Writer
```
...
skill = load_skill("budgetai/Budget-Proposal-Writer")
...
```json
{
  "skill": "budgetai/Budget-Proposal-Writer",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("budgetai/Budget-Proposal-Writer", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("budgetai/Budget-Proposal-Writer"),
...
```bash
# Clone the skill
skillshub clone budgetai/Budget-Proposal-Writer
cd Budget-Proposal-Writer
