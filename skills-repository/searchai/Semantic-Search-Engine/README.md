...
**Semantic Search Engine** is a production-ready agent skill designed for NLP workflows. Built with industry best practices, it provides reliable, efficient, and scalable capabilities for modern AI applications.
...
skillshub install searchai/Semantic-Search-Engine

# Or install via pip
pip install skillshub-semantic-search-engine

# Or install via npm
npm install @skillshub/semantic-search-engine
```
...
skill = load_skill("searchai/Semantic-Search-Engine")
...
```json
{
  "skill": "searchai/Semantic-Search-Engine",
  "version": "1.0.0",
  "model": {
    "provider": "openai",
    "name": "gpt-4o",
...
skill = load_skill("searchai/Semantic-Search-Engine", config=config)
...
agent = Agent(
    name="My Agent",
    skills=[
        load_skill("searchai/Semantic-Search-Engine"),
...
```bash
# Clone the skill
skillshub clone searchai/Semantic-Search-Engine
...
**Built with ❤️ by [searchai](https://skillshub.dev/searchai) on [SkillsHub](https://skillshub.dev)**
